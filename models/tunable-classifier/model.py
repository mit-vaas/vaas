...
def get_model(num_classes, dims):
	...
	for dim in dims:
		...
		fimage = keras.layers.Lambda(lambda im: tf.image.resize_images(im, [dim[1], dim[0]]))(prev_im)
	...
	for i, fimage in enumerate(fimages[1:]):
		...
		feature_conv = keras.layers.Lambda(lambda l: l[0]*l[1][:, i])([feature_conv, weight_input])
	...
	for i, conv in enumerate(conv_outputs):
		...
		out = keras.layers.Lambda(lambda l: l[0]*l[1][:, i])([out, weight_input])
